...
<div class="login">
    <div class="login__action">
        <div class="login__card" style="text-align: center; max-width: 400px; margin: 0 auto;">
            ...
            <div class="login__card__content" id="totp-setup-container">
            ...
            methods: {
                async verifyCode() {
                    if (this.code.length !== 6) {
                        this.error = '<?= i::__('El código debe tener 6 dígitos.') ?>';
                        return;
                    }
                    this.error = '';
                    this.loading = true;
                    ...
                    try {
                        const response = await fetch('<?= $app->createUrl('auth', 'setup_totp') ?>', {
                            method: 'POST',
                            headers: {
                        ...
                        if (data.success) {
                            this.success = true;
                            setTimeout(() => {
                                window.location.href = '<?= $app->createUrl('panel', 'index') ?>';
                            }, 2000);
                        } else {
                            this.error = data.message || '<?= i::__('Error al verificar.') ?>';
                        }
                    } catch (e) {
                        this.error = '<?= i::__('Error de conexión') ?>';
                        console.error(e);
                    } finally {
                        this.loading = false;
                    }
