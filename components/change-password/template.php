...
<div class="change-password">
    <div v-if="myAccount" class="mfa-section-wrapper" style="margin-bottom: 24px;">
        ...
        <!-- MANDATORY MODE -->
        <div v-if="mfaMandatory" class="field col-12 mfa-section" style="padding: 16px; background: #fff; border: 1px solid #eee; border-radius: 4px;">
        ...
        <!-- OPTIONAL MODE -->
        <div v-else class="field col-12 mfa-section" style="padding: 16px; background: #fff; border: 1px solid #eee; border-radius: 4px;">
            <div style="display: flex; align-items: center; gap: 10px;">
                <input type="checkbox" id="mfaToggle" v-model="mfaEnabled" @change="toggleMFA" style="width: auto; margin: 0; cursor: pointer;">
                <label for="mfaToggle" style="margin: 0; cursor: pointer; font-weight: 500; font-size: 14px;"><?= i::__('Activar Autenticación de Dos Factores (MFA)') ?></label>
                 <!-- Status Badge (Optional) -->
                 <span v-if="mfaEnabled" style="font-size: 11px; background: #28a745; color: white; padding: 2px 6px; border-radius: 4px; margin-left: 8px;">
                    <?= i::__('Activado') ?>
                </span>
                <span v-else style="font-size: 11px; background: #6c757d; color: white; padding: 2px 6px; border-radius: 4px; margin-left: 8px;">
                    <?= i::__('Desactivado') ?>
                </span>
            </div>
